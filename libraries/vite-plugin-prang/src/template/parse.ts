...
function isCompatEnabled(key: CompilerDeprecationTypes, context: MergedParserOptions | TransformContext): boolean {
...
const tokenizer = new Tokenizer(stack, {
    ...
    onattribend(quote, end) {
        if (currentOpenTag && currentProp) {
            ...
            if (currentProp.type !== NodeTypes.DIRECTIVE || currentProp.name !== 'pre') {
...
function onCloseTag(el: ElementNode, end: number, isImplied = false) {
    ...
    if (tokenizer.inXML && (stack[0] ? stack[0].ns : currentOptions.ns) === Namespaces.HTML) {
    ...
    if (__COMPAT__) {
        ...
        if (__DEV__ && isCompatEnabled(CompilerDeprecationTypes.COMPILER_V_IF_V_FOR_PRECEDENCE, currentOptions)) {
            let hasIf = false;
            let hasFor = false;
            for (let i = 0; i < props.length; i++) {
                const p = props[i];
                ...
                if (hasIf && hasFor) {
                    warnDeprecation(CompilerDeprecationTypes.COMPILER_V_IF_V_FOR_PRECEDENCE, currentOptions, el.loc);
                    break;
                }
            }
        }
        ...
        if (
            !tokenizer.inSFCRoot &&
            isCompatEnabled(CompilerDeprecationTypes.COMPILER_NATIVE_TEMPLATE, currentOptions) &&
            el.tag === 'template' &&
            !isFragmentTemplate(el)
        ) {
            __DEV__ && warnDeprecation(CompilerDeprecationTypes.COMPILER_NATIVE_TEMPLATE, currentOptions, el.loc);
            // unwrap
            const parent = stack[0] || currentRoot;
...
function isFragmentTemplate({ tag, props }: ElementNode): boolean {
    if (tag === 'template') {
        for (let i = 0; i < props.length; i++) {
            if (props[i].type === NodeTypes.DIRECTIVE && specialTemplateDir.has((props[i] as DirectiveNode).name)) {
                return true;
            }
        }
    }
        ...
        if (p.type === NodeTypes.ATTRIBUTE) {
            if (p.name === 'is' && p.value) {
                if (p.value.content.startsWith('vue:')) {
                    ...
                    __COMPAT__ &&
                    checkCompatEnabled(CompilerDeprecationTypes.COMPILER_IS_ON_ELEMENT, currentOptions, p.loc)
                ) {
                    return true;
                }
            }
...
function condenseWhitespace(nodes: TemplateChildNode[], tag?: string): TemplateChildNode[] {
